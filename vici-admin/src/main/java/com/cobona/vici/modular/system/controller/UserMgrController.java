...
import com.cobona.vici.common.constant.dictmap.UserDict;
...
import com.cobona.vici.core.datascope.DataScope;
import com.cobona.vici.core.db.Db;
...
public class UserMgrController extends BaseController {
    ...
    public Object changePwd(@RequestParam String oldPwd, @RequestParam String newPwd, @RequestParam String rePwd) {
    	...
    	if (!newPwd.matches(regex)) {
            throw new BussinessException(BizExceptionEnum.NEW_PWD_FMT_NOT_RIGHT);
        }
    ...
    @ResponseBody
    public Object checkPwd(@RequestParam String oldPwd, @RequestParam String newPwd, @RequestParam String rePwd ) {
    ...
    //@RequiresPermissions("/mgr/role_assign")  //利用shiro自带的权限检查
    @Permission
    @RequestMapping("/setAuthority/{userId}")
    public String setAuthority(@PathVariable Integer userId, Model model) {
        if (ToolUtil.isEmpty(userId)) {
            throw new BussinessException(BizExceptionEnum.REQUEST_NULL);
        }
        model.addAttribute("userID", userId);
        model.addAttribute("userName", ConstantFactory.me().getUserNameById(userId));
        return PREFIX + "user_set_authority.html";
    }
    ...
    @BussinessLog(value = "配置权限", key = "userId,ids", dict = DeptDict.class)
    @Permission(Const.ADMIN_NAME)
    @ResponseBody
    public Tip saveAuthority(@RequestParam("userId") Integer userId, @RequestParam("ids") String ids) {
        if (ToolUtil.isOneEmpty(userId)) {
            throw new BussinessException(BizExceptionEnum.REQUEST_NULL);
        }
        Integer addUid = ShiroKit.getUser().getId();
        this.userrelationService.saveAuthority(userId,addUid, ids);
        return SUCCESS_TIP;
    }
}
