...
OptLog.detail = function () {
    if (this.check()) {
        var ajax = new $ax(Feng.ctxPath + "/super/detail/" + this.seItem.id, function (data) {
...
OptLog.delLog = function () {
    Feng.confirm("是否清空所有日志?",function(){
        var ajax = Feng.baseAjax("/super/delLog","清空日志");
        ajax.start();
...
$(function () {
    ...
    var table = new BSTable(OptLog.id, "/super/list", defaultColunms);
